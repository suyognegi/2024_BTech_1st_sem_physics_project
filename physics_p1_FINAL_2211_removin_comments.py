...
class EightPageGraph(QMainWindow):
    ...
    def plot_graph(self, eq1, eq2):
        ...
        try:
            ...
            y_vals_eq1 = [float(expr1.subs('x', val)) for val in x_vals]
            y_vals_eq2 = [float(expr2.subs('x', val)) for val in x_vals]

            # Plot the graphs
            ax.plot(x_vals, y_vals_eq1, label=f"$y = {latex(expr1)}$", color='darkred')
            ax.plot(x_vals, y_vals_eq2, label=f"$y = {latex(expr2)}$", color='darkblue')

            # Highlight the region between the curves
            ax.fill_between(x_vals, y_vals_eq1, y_vals_eq2, where=(np.array(y_vals_eq1) >= np.array(y_vals_eq2)), interpolate=True, color='darkorange', alpha=0.8)
            ...
            # Save the figure
            fig.savefig("physics_p8_graph.png", dpi=140, transparent=True)
            plt.close(fig)
...
class NineGraphPlot(QMainWindow):
    ...
    def plot_graph(self, trig_type, trig_value):
        ...
        # Create the plot
        plt.figure()
        plt.plot(x, y, label=f'{trig_type.capitalize()} curve', linewidth=1.5, color='#1f77b4')

        try:
            theta = self.calculate_inverse_trig(trig_type, trig_value)
            theta_radians = math.radians(theta)
            y_value_at_theta = trig_value
            plt.plot(theta_radians, y_value_at_theta, 'ro', label=f'{trig_type.capitalize()}(θ) = {trig_value:.3f}')
            plt.axvline(x=theta_radians, color='green', linestyle='--', label=f'θ = {theta:.2f}°')
        except ValueError as e:
        ...
        if trig_type in ["tan", "csc", "sec", "cot"]:
            plt.ylim(min(-10, trig_value - y_padding), max(10, trig_value + y_padding))
        else:
            plt.ylim(min(-1.5, trig_value - y_padding), max(1.5, trig_value + y_padding))

        # Set transparent background
        plt.gcf().patch.set_alpha(0.0)
        plt.gca().set_facecolor((0, 0, 0, 0))  # Transparent background for the plot area

        # Save the plot to a file
        plot_file = "trig_plot.png"
        plt.savefig(plot_file, format='png', transparent=True)
        plt.close()  # Close the plot after saving

        # Display the image in QLabel (label_4)
        self.set_image_to_label(plot_file)

    def set_image_to_label(self, image_path):
        # Load the image from the file
        pixmap = QPixmap(image_path)

        # Set the image to label_4
        self.label_4.setPixmap(pixmap)
        self.label_4.setScaledContents(True)  # Ensure the image scales properly within the label
