...
class NineGraph(QMainWindow):
    ...
    def setup_ui(self):
        self.pushButton_2.clicked.connect(lambda: self.handle_trig_function("sin"))
        self.pushButton_3.clicked.connect(lambda: self.handle_trig_function("cos"))
        self.pushButton_4.clicked.connect(lambda: self.handle_trig_function("tan"))
        self.pushButton_5.clicked.connect(lambda: self.handle_trig_function("csc"))
        self.pushButton_6.clicked.connect(lambda: self.handle_trig_function("sec"))
        self.pushButton_7.clicked.connect(lambda: self.handle_trig_function("cot"))
    ...
    def handle_trig_function(self, trig_type):
        """Handle button clicks for trigonometric functions."""
        theta = self.trig_values.get(trig_type)
        if theta == "None":
            theta = self.prompt_for_theta_or_value(trig_type)

        if theta is not None:
            self.trig_values[trig_type] = theta
            self.call_window = NineGraphPlot(theta, trig_type)
            widget.addWidget(self.call_window)
            widget.setCurrentIndex(widget.currentIndex() + 1)

    def prompt_for_theta_or_value(self, trig_type):
        """Prompt user to input theta or trigonometric value."""
        msg = QMessageBox()
        msg.setIcon(QMessageBox.Warning)
        msg.setWindowTitle(f"Missing {trig_type.capitalize()} Value")
        msg.setText(f"The {trig_type.capitalize()} value is missing. Choose an option below:")
        ...
        if msg.clickedButton() == theta_button:
            return self.get_theta_from_user(trig_type)
        elif msg.clickedButton() == value_button:
            return self.get_value_from_user(trig_type)
        return None

    def get_theta_from_user(self, trig_type):
        theta, ok = QInputDialog.getText(self, f'Input {trig_type.capitalize()} Theta', 'Enter angle (degrees):')
        if ok and theta:
            try:
                theta_radians = math.radians(float(theta))
                trig_value = self.calculate_trig(trig_type, theta_radians)
                print(f"{trig_type.capitalize()}({theta}°) = {trig_value}")
                return theta
            except ValueError:
                print(f"Invalid input for {trig_type.capitalize()} Theta.")
        return None

    def get_value_from_user(self, trig_type):
        value, ok = QInputDialog.getText(self, f'Input {trig_type.capitalize()} Value',
                                         f'Enter {trig_type.capitalize()} value:')
        if ok and value:
            try:
                value = float(value)
                theta = self.calculate_inverse_trig(trig_type, value)
                print(f"{trig_type.capitalize()} Value: {value}, Theta = {theta}°")
                return theta
            except ValueError:
                print(f"Invalid input for {trig_type.capitalize()} Value.")
        return None

    def calculate_trig(self, trig_type, theta):
        """Calculate trigonometric value based on type."""
        if trig_type in ["sin", "cos", "tan"]:
            return getattr(math, trig_type)(theta)
        elif trig_type == "csc":
    ...
    def calculate_inverse_trig(self, trig_type, value):
        """Calculate inverse trigonometric value based on type."""
        if trig_type == "sin":
...
class NineGraphPlot(QMainWindow):
    def __init__(self, value, trig_type):
        ...
        # Plot the graph with the given value and trig_type
        try:
            self.plot_graph(trig_type, float(value))
    ...
    def calculate_inverse_trig(self, trig_type, value):
        """Calculate the angle (theta) in degrees from the trigonometric value."""
        if trig_type == "sin":
            return math.degrees(math.asin(value))
        elif trig_type == "cos":
            return math.degrees(math.acos(value))
    ...
    def plot_graph(self, trig_type, trig_value):
        """Plot the trigonometric function with the given value marked."""
        ax = self.canvas.figure.add_subplot(111)

        # Define the x-axis range for plotting (from -2*pi to 2*pi)
        x = np.linspace(-2 * np.pi, 2 * np.pi, 1000)

        # Clear previous plot
        ax.clear()

        # Determine the y values for the given trig_type
        trig_functions = {
            "sin": np.sin(x),
            "cos": np.cos(x),
            "tan": np.tan(x),
        ...
        y = trig_functions.get(trig_type)
        ax.plot(x, y, label=f'{trig_type.capitalize()} curve', linewidth=1.5, color='#1f77b4')

        # Calculate and mark the corresponding angle (theta) for the trig_value
        try:
            theta = self.calculate_inverse_trig(trig_type, trig_value)  # theta in degrees
            theta_radians = math.radians(theta)  # Convert to radians for plotting

            # Plot the corresponding point
            y_value_at_theta = trig_value
            ax.plot(theta_radians, y_value_at_theta, 'ro', label=f'{trig_type.capitalize()}(θ) = {trig_value:.3f}')

            # Draw a vertical line at the marked point
            ax.axvline(x=theta_radians, color='green', linestyle='--', label=f'θ = {theta:.2f}°')

        except ValueError as e:
            print(f"Error in marking the angle: {e}")

        # Customize the graph
        ax.set_title(f'Graph of {trig_type.capitalize()} Function')
        ax.set_xlabel('Angle (radians)')
        ax.set_ylabel(f'{trig_type.capitalize()} Value')
        ax.legend()
        ax.grid(True)

        # Dynamically set y-axis limits based on trig_value and trig_type
        y_padding = 2  # Padding for better visualization
        if trig_type in ["tan", "csc", "sec", "cot"]:
            ax.set_ylim(min(-10, trig_value - y_padding), max(10, trig_value + y_padding))
        else:
            # For sin and cos, limit range to [-1, 1] or around trig_value if it exceeds normal range
            ax.set_ylim(min(-1.5, trig_value - y_padding), max(1.5, trig_value + y_padding))
